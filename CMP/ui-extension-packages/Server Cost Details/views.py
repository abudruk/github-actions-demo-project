...
def cost_details_tab(request, obj_id):
    ...
    return render(request, 'server_cost_details/templates/cost_details.html', dict(
...
def _calc_totals(cost):
    ...
    for d in cost['disks']:
        ext_cost_total += d['size'] * cost['detail']['disk']['unit_cost'] * d['multiplier']
...
def _update_disks(cost, server):
    for disk in server.disks.all():
        ...
        cost['disks'].append({
            'name': '{} ({})'.format(disk.name, storage_type) if storage_type else disk.name,
            'size': disk.disk_size,
            'multiplier': multiplier,
            'ext_cost_display': render_rate(unit_cost * disk.disk_size * multiplier)
        })
...
def _update_osbuild_rate(cost, server):
    ...
    osb_rate = osb.osbuildrate_set.filter(environment=server.environment).first()
    if not osb_rate:
    ...
    if osb_rate and osb_rate.rate:
        ...
        # There's no rate associated with this OS Build for this environment/globally
        return
...
def _update_app_rates(cost, server):
    for app in server.applications.all():
        ...
        app_rate = app.applicationrate_set.filter(environment=server.environment).first()
        if not app_rate:
...
def _update_param_rates(cost, server):
    for param in server.custom_field_values.all():
        ...
        cfv_rate = param.field.customfieldrate_set.filter(environment=server.environment).first()
        if not cfv_rate:
            cfv_rate = param.field.customfieldrate_set.filter(environment=None).first()
