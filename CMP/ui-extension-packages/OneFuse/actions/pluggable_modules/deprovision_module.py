...
def run(job, **kwargs):
    ...
    if server:
        ...
        matching_props = utilities.get_matching_properties(
            'OneFuse_PluggableModule_', properties_stack)
        if len(matching_props) > 0:
            ...
            for matching_prop in matching_props:
                ...
                logger.debug(f'Preparing to deprovision Pluggable Module for '
                             f'hostname: {hostname}, suffix: '
                ...
                ofm = CbOneFuseManager(onefuse_endpoint, VERIFY_CERTS,
                                       logger=logger)
                set_progress(f'Calling OneFuse to deprovision Module '
                             f'Managed Object. ID: {id}')
                try:
                    ofm.deprovision_module(id)
                except OneFuseError as err:
                    if err.args[0].find('is already archived') > -1:
                        logger.info('Object is already archived. Continuing.')
                    else:
                        raise
                logger.info("Pluggable Module deletion completed.")
            return "SUCCESS", "", ""
        else:
            logger.info("No OneFuse_PluggableModule_ properties found on the "
                        "server, Deprovision Pluggable Modules will not be "
                        "executed.")
    else:
