        ...
        if len(matching_props) > 0:
            ...
            for matching_prop in matching_props:
                ...
                ofm = CbOneFuseManager(onefuse_endpoint, VERIFY_CERTS,
                                       logger=logger)
                set_progress(f'Calling OneFuse to delete Ansible Tower '
                             f'Managed Object. ID: {id}')
                ofm.deprovision_ansible_tower(id)
                logger.info("Ansible Tower deletion completed.")
            return "SUCCESS", "", ""
        else:
            logger.info("No OneFuse_AnsibleTower_ properties found on the "
                        "server, Deprovision Ansible Tower will not be "
                        "executed.")
    else:
