...
class VeeamManager(object):
    ...
    def api_get(self, url):
        ...
        return requests.get(url=uri, headers=header)
    ...
    def take_backup(self, template, context):
        ...
        try:
            ...
            logger.info("Error occurred while trying to parse result")
    ...
    def get_summary(self):
        ...
        header = {'X-RestSvcSessionId': self.session_id,
                  'Accept': "Application/json"}
        backup_servers = requests.get(url=url_backup_servers, headers=header)
        url_jobs = self.connection_info.format_url(auth=False) + "/api/jobs"
        jobs = requests.get(url=url_jobs, headers=header)
        url_backups = self.connection_info.format_url(
            auth=False) + "/api/backups"
        backups = requests.get(url=url_backups, headers=header)
        url_agent_jobs = self.connection_info.format_url(
            auth=False) + "/api/agents/jobs"
        agent_jobs = requests.get(url=url_agent_jobs, headers=header)
