import { LoaderClass, DATA_BACKUP_HOURS, numberFormatter,
  ...
  POTENTIAL_SAVINGS, lockModal, unlockModal,
  sanitizeHtml } from './common.js';
...
$(document).ready(function () {
  if (SERVER_DATA.validation == true) {
    $('#server-tab .errorMessageModal').modal('hide');
    $('#server-container').css('filter', 'none');
    $('#server-tab .go-to-admin').addClass('hidden');
    $('#server-tab .go-to-rh').addClass('hidden');
    $('#server-tab .no-data').addClass('hidden');
    $('#server-tab .overviewButton').addClass('hidden');
    unlockModal();
    setDateRange();
    getCostSummary();
    getPotentialSavings();
    ...
    $(document).on('click', '#go-to-details', function (e) {
      $("a[href='#tab-details']")[0].click();
    });
  }
  else {
    ...
    $('#server-tab .go-to-admin').removeClass('hidden');
    $('#server-tab .go-to-rh').addClass('hidden');
    $('#server-tab .no-data').addClass('hidden');
  }
});
  ...
  $('#graph-div').highcharts({
    ...
    yAxis: {
      ...
      labels: {
        enabled: true,
        formatter: function () {
          return SERVER_DATA.acc_currency + this.value;
        }
      }
    },
    ...
    tooltip: {
      formatter: function () {
          return `${this.x}<br />${this.series.name}: <b>${SERVER_DATA.acc_currency}${numberFormatter(this.y)}</b>`;
        }
    },
      ...
      if (chart.series.length != 0) {
        ...
        $.each(chart.series, function (j, data) {
          if (chart.series.length == 1) {
            ...
            legend_str = legend_str + `<div class="item col-sm-6"><span class="serieName"><span class="symbol" style="background-color: ${data.color}"></span><div style="display: inline-block;"><div style="float: left;">${data.name}</div><div class="legend-value">${SERVER_DATA.acc_currency}${numberFormatter(data.yData.reduce((a, b) => a + b, 0))}</div></div></span></div>`;
    ...
    success: function (response) {
      if (response.result == "NO RH") {
        ...
        if (response.result == "NO RH") {
          ...
          $('#lsd-cost').html(`<span class="acc-currency">${SERVER_DATA.acc_currency}</span>${numberFormatter(response.result.last_7_days_cost)}`);
          ...
          if (response.result.previous_month_cost != 0) {
            ...
            $('#pm-cost').html(`<span class="acc-currency">${SERVER_DATA.acc_currency}</span>${numberFormatter(response.result.previous_month_cost)}`);
          }
          else {
            $('#previous-month-cost').hide();
          }

          $('#cm-cost').html(`<span class="acc-currency">${SERVER_DATA.acc_currency}</span>${numberFormatter(response.result.current_month_estimated_cost)}`);
          $('#t-cost').html(`<span class="acc-currency">${SERVER_DATA.acc_currency}</span>${numberFormatter(response.result.total_cost.total_cost)}`);
  ...
  if(SERVER_DATA.type_slug == 'aws'){
    $.ajax({
      ...
      success: function (response) {
        ...
        $('#total-graph-cost').html(`<span class="acc-currency">${SERVER_DATA.acc_currency}</span>${numberFormatter(totalGraphCost)}`);
          ...
          response.result.forEach(element => {
            if (element.count != 0) {
              $('#example tbody').append(`<tr>
                                          ...
                                          <td>${element.count}</td>
                                          <td><strong><span class="acc-currency">${SERVER_DATA.acc_currency}</span>${numberFormatter(parseFloat(element.cost_sum))}</strong></td>
                                      </tr>`)
              totalSavings = totalSavings + parseFloat(element.cost_sum);
            }
          });
        }
        $('#total-savings').html(`<span class="acc-currency">${SERVER_DATA.acc_currency}</span>${numberFormatter(totalSavings)}`);
...
function JsonToCSV(jsonArray){
  ...
  let filterFields = {
    ...
    "Total Cost": SERVER_DATA.acc_currency + numberFormatter(totalGraphCost),
  };
...
function downloadPDF(csvStr) {
  ...
  doc.text(pageGap+10, pageGap+40, `Total: ${SERVER_DATA.acc_currency}${numberFormatter(totalGraphCost)}`);
