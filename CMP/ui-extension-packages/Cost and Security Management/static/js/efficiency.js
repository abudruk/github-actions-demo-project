import {
  ...
  GCP_SERVICE_TYPE_MATCH, GCP_SERVICE_TAG_MAP,
  SERVICE_WISE_METRICS, AWS_COLUMN_TO_SHOW,
  AWS_COLUMN_TO_SHOW_1, GCP_COLUMN_TO_SHOW,
...
var accountSelect = dropdownValue('accounts');
...
var SERVICE_TYPE_TAG_MAP;

const KUMO_WEB_HOST_EFFI = $('#tab-efficiency #host_details').data('host');
const RESOURCE_HANDLER = {
  handlerId: $('#handler_details_efficiency').data('handlerid'),
  handlerType: $('#handler_details_efficiency').data('handler'),
  handlerCurrency: $('#handler_details_efficiency').data('acurrency'),
  handlerNormalId : ($('#handler_details_efficiency').data('handler') == "GCP") ?
                      JSON.parse($('#handler_details_efficiency').data('normalid').replace(/'/g, '"'))
                      : String($('#handler_details_efficiency').data('normalid')),
  normalAdapterId: $('#handler_details_efficiency').data('normaladapter'),
  validation: $('#handler_details_efficiency').data('validation'),
}
...
$(document).ready(function () {
  if (RESOURCE_HANDLER.handlerType === 'AWS') {
    SERVICE_TYPE_MAP = AWS_SERVICE_TYPE_MATCH
    SERVICE_TYPE_TAG_MAP = AWS_SERVICE_TAG_MAP
  }
  else if (RESOURCE_HANDLER.handlerType === 'GCP') {
    SERVICE_TYPE_MAP = GCP_SERVICE_TYPE_MATCH
    SERVICE_TYPE_TAG_MAP = GCP_SERVICE_TAG_MAP
  }
  else if (RESOURCE_HANDLER.handlerType === 'Azure') {
    SERVICE_TYPE_MAP = AZURE_SERVICE_TYPE_MATCH
    SERVICE_TYPE_TAG_MAP = AZURE_SERVICE_TAG_MAP
  }

  if ($("#cost-accounts").val() == "") {
    accountSelect = RESOURCE_HANDLER.handlerNormalId;
  }
  else {
    accountSelect = dropdownValue('accounts');
  }

  if ((KUMO_WEB_HOST_EFFI != "") && (RESOURCE_HANDLER.validation == "True")) {
    if ($('#handler_details_efficiency').data('handler') == "GCP") {
    ...
    if (RESOURCE_HANDLER.handlerNormalId != "") {
      ...
      if (RESOURCE_HANDLER.handlerType === 'GCP') {
        RESOURCE_HANDLER.handlerNormalId.forEach(ACC => {
          $("#cost-accounts, #select-accounts").append(`<option value="${ACC}">${ACC}</option>`);
        });
      }

      applySelectize('#cost-services, #select-services, #cost-lifecycle, #select-lifecycle, #cost-accounts, #select-accounts', false);

      $(document).on('click', '.clickable-call', function (e) {
        $('#listServicesModal').modal('show');
        accountSelect = $('#cost-accounts').val();
      ...
      $(document).on('click', '#effi-close', function (e) {
        if (($('#select-accounts').val() == $('#cost-accounts').val()) &&
            ($('#select-regions').val() == $('#cost-regions').val()) &&
        ...
        else if (($('#select-accounts').val() != "") || ($('#select-regions').val() != "") ||
                 ($('#select-services').val() != "") || ($('#select-tags').val() != "") ||
                 ($('#select-lifecycle').val() != "") || ($('#select-values').val() != "")) {
          clearAndResetSelectize();
...
function clearAndResetSelectize() {
  ...
  if (RESOURCE_HANDLER.handlerType != "GCP") {
    ...
    resetSelectizeValue(`#select-accounts`);
  }
    ...
    switch (type) {
      case 'idle_vm':
        for (var i = 0; i < response.length; i++) {
          let data = [];
          let additionalInfo = getAdditionalInfo(response[i])
          data.push(response[i].adapter_name || 'N/A')
          data.push(response[i].name || 'N/A')
...
async function getRegions() {
  ...
  if (RESOURCE_HANDLER.handlerType == "GCP") {
    payload.adapter_id = accountSelect;
  }
...
function getServicesList() {
  ...
  if (RESOURCE_HANDLER.handlerType == "GCP") {
    if (accountSelect == "") {
      payload.adapter_id = RESOURCE_HANDLER.handlerNormalId;
      payload.adapter_ids = RESOURCE_HANDLER.handlerNormalId;
    } else {
      payload.adapter_id = [accountSelect];
      payload.adapter_ids = [accountSelect];
      payload.provider_account_id = [accountSelect];
    }
  }
  ...
  $.post(URL, JSON.stringify(payload), (response) => {
    ...
    if (noNormal) {
      $('#efficiency-tab .generalError p').text(`
        No normal adapter found in CSMP. Please create one
        in CSMP account and try again later!
      `);
      $('#efficiency-tab .errorMessageModal').modal('show');
      lockModal();
      $('#form-cost').css('filter', 'blur(10px)');
      ...
      if (Object.keys(response.service_type_summary).length != 0) {
        ...
        if ($('#listServicesModal').hasClass('in')) {
          ...
          $('#potential-cost').text(`${RESOURCE_HANDLER.handlerCurrency}${numberFormatter(potentialBenefit) || 0}`);
...
function applyModalFilters() {
  accountSelect = $('#select-accounts').val();
...
function appendTagListBox() {
  ...
  if ($('#listServicesModal').hasClass('in')) {
    ...
    selectedTagList.forEach(function (elm, i) {
      $("#tagSelectListBox").append(sanitizeHtml(`
    ...
    selectedTagList.forEach(function (elm, i) {
      $("#tagListBox").append(sanitizeHtml(`
        <div class="btn btn-success tagList">
        ${elm.tag_key} : ${elm.tag_value}
        <span class="fa fa-times remove-tag-icon testRemove" data-value=${i}></span>
        </div>`
...
function setSelectizeValue() {
  ...
  if (RESOURCE_HANDLER.handlerType != "GCP") {
    ...
    $(`#select-accounts`).data('selectize').setValue(accountSelect, true);
  }
...
function getServiceDetail(e, selectState) {
  ...
  $.post(URL, JSON.stringify(payload), (response) => {
    ...
    if (adapterNameMaster) {
      $('.AdapterNameField').show();
      $('#adapterNameMaster').text(adapterNameMaster);
    } else {
      $('.AdapterNameField').hide();
    }
...
function getCSV(service) {
  ...
  if (RESOURCE_HANDLER.handlerType == "GCP") {
    ...
    payload = {
      noloader: "",
      adapter_id: accountSelect,
      region_id: regionSelect,
      service_type: service,
      lifecycle: "",
      service_adviser_klass: `ServiceAdviser::${RESOURCE_HANDLER.handlerType}`,
      sort_by: "cost_by_hour",
      sort: "ASC",
      count: "true",
      public_snapshot: "false",
      tags: JSON.stringify(selectedTagList),
      per_page: 10,
      ec2_status: false,
      provider_account_id: accountSelect,
    }
  }
  else {
  ...
  const URL = '/xui/kumo/api/csv_download_for_rh_json/';

  $.post(URL, JSON.stringify(payload), (response) => {
    var blob = new Blob([response]);
    var link = document.createElement('a');
    link.href = window.URL.createObjectURL(blob);
    link.download = "recommendation.csv";
    link.click();
    // loaderObjEffi.hide();
  });
}
...
function loadServices(listServices, serviceCategory) {
          ...
          if (ser['type'] == "ignore_services") {
            $('#cost-accordion-table tbody').append(
              `<tr>
                  ...
                  <td>${ser['count']}</td>
                  <td><span class="acc-currency">${RESOURCE_HANDLER.handlerCurrency}</span>${numberFormatter(parseFloat(ser['cost_sum']))}</td>
  ...
  mainTable = $('#cost-accordion-table').dataTable({
    responsive: true,
    searching: false,
    paging: true,
    info: false,
    "bDestroy": true,
    "lengthMenu": [[7, 14, 20, 50, -1], [7, 14, 20, 50, "All"]]
  });
}
...
$("#cost-accounts, #cost-services, #cost-regions, #cost-lifecycle").on("change", function () {
  if ($("#cost-accounts").val() == "") {
    accountSelect = "";
  }
  else {
...
$("#cost-tags, #select-tags").on("change", function () {
  ...
  if (RESOURCE_HANDLER.handlerType == "GCP") {
    ...
    if ($('#listServicesModal').hasClass('in')) {
      ...
      labelKey = $("#select-tags").val()
    }
    else {
      labelKey = $("#cost-tags").val()
    }

    if (labelKey != "") {
      let payload = {
        rh_id: RESOURCE_HANDLER.handlerId,
        label_key: dropdownValue('tags'),
        provider_account_id: RESOURCE_HANDLER.handlerNormalId,
      }
...
function getModalGraphData() {
  ...
  payload.provider_account_id = RESOURCE_HANDLER.handlerNormalId;
