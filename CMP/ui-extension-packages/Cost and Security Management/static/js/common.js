...
export function showAlert(message, alertType) {
  $('#alert-box').html('');
  $('#alert-box').append(`
    <div class="alert alert-dismissable ${alertType}" style="opacity: 1;">
      <button type="button" class="close" data-dismiss="alert" title="Dismiss this notification"><i class="fas fa-times"></i>&nbsp</button>
      ${message}
    </div>
  `);
  document.getElementById('alert-box').scrollIntoView({ behavior: 'smooth', block: 'center' });
}
...
export const GCP_SERVICE_TYPE_MATCH = {
...
export const GCP_SERVICE_TAG_MAP = {
  "idle_vm": "idle_vm",
...
export const GCP_COLUMN_TO_SHOW = [
  { title: "Account" },
  { title: "Service Name(ID)" },
  { title: "State" },
  { title: "Region" },
