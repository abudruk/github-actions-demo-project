...
def _set_report_thumbnails():
    ...
    dst_dir = os.path.join(PROSERV_DIR, "static", "openstack_reports", "images")
...
@report_extension(
    ...
    thumbnail="images/monthly_performance_report.png",
    delegate=OpenStackReportsDelegate
)
def monthly_performance_report(request):
    servers = []
    resource_handlers = OpenStackHandler.objects.all()
    for resource_handler in resource_handlers:
        print(f"resource_handler {resource_handler}")
        rh_servers = resource_handler.server_set.exclude(status="HISTORICAL")
        for server in rh_servers:
            print(f"server {server}")
            server_inventory, response_status = get_vm_basic_inventory(resource_handler, server)
            if response_status:
                servers.append(server_inventory)
    start_date = datetime.now() - timedelta(days=30)
    end_date = datetime.now()
    return render(request, 'openstack_reports/templates/monthly_performance_report.html', {
...
@report_extension(
    ...
    thumbnail="images/optimization_report.png",
    delegate=OpenStackReportsDelegate
)
def monthly_optimization_report(request):
    ...
    return render(request, 'openstack_reports/templates/optimization_report.html', {
        "pagetitle": "OpenStack Optimization Report",
        "snapshots": snapshots
    })
...
def get_utilization_report_data(request):
    if request.method == "POST":
        ...
        wrapper = VMDetailsWrapper(resource_handler.cast(
        ), resource_handler.ip, server.resource_handler_svr_id, resource_handler.protocol)
