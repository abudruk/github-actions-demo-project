...
class GCPAdmin(object):
    ...
    def __init__(self, environment):
        ...
        self.project_name = gcp_project.name
        try:
    ...
    def get_client(self, serviceName, version="v1"):
        return build(serviceName, version, cache_discovery=False, credentials=self.credentials)
...
def run(job=None, logger=None, **kwargs):
    ...
    project = GCPProject.objects.get(name=project_name)
    environment = project.environment
    gcp_admin = GCPAdmin(environment)
    client = gcp_admin.storage_client

    # Delete the Database
    set_progress(f"Attempting to create delete database: {db_name}")
