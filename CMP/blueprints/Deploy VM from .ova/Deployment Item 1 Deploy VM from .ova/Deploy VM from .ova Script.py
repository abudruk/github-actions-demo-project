...
def generate_options_for_env_id(**kwargs):
    envs = Environment.objects.filter(
        resource_handler__resource_technology__name="VMware vCenter").values("id", "name")
...
def generate_options_for_network_name(control_value=None, **kwargs):
    ...
    possible_options = [net_cfv.value.network for net_cfv in possible_option_info.get('options')]
...
def determine_hostname(hostname):
    ...
    """
    case_sensitive = GlobalPreferences.get_preference('hostname_case_sensitivity', False)
...
def create_server_record(job, rh, env, vmname):
    ...
    server = Server(hostname=vmname, group=group, owner=job.owner, environment=env,
                    resource_handler=rh, status="PROV", power_status="POWEROFF")
    server.save()
...
def run(job, *args, **kwargs):
    ...
    ovftool_cmd = (
        f"ovftool -ds={datastore} --X:logToConsole --X:logLevel=info --network='{network_name}' "
    ...
    # set_progress("Script completed successfully", detailed_message=stdout)
