...
def generate_options_for_obj_name(server=None, **kwargs):
    ...
    options = []
    options.append(('', 'Select Object/File Name to be deleted'))
    if resource:
        aws: AWSHandler = AWSHandler.objects.get(id=resource.aws_rh_id)

        bucket_name = resource.s3_bucket_name

        client = aws.get_boto3_client(resource.s3_bucket_region, 's3')
