"""
Creates an PostgreSQL in Azure.
"""
...
def create_custom_fields_as_needed():
    ...
    CustomField.objects.get_or_create(
        name="azure_database_user",
        type="STR",
        defaults={
            "label": "Azure Database User",
            "description": "Used by the Azure blueprints",
            "show_as_attribute": True,
        },
    )
    ...
    CustomField.objects.get_or_create(
        name='db_address', 
        defaults={
            'label': 'Azure PostgreSQL Database address', 
            'type': 'STR',
            'description': 'Used by the Azure blueprints'
    ...
    CustomField.objects.get_or_create(
        name='db_port', 
        defaults={
            'label': 'Azure PostgreSQL Database port', 
            'type': 'STR',
            'description': 'Used by the Azure blueprint'
        }, 
        show_as_attribute=True
    )
...
def run(job, **kwargs):
    ...
    try:
        ...
        set_progress('Azure ResourceNotFoundError: {}'.format(e))
    else:
    ...
    server_created = async_server_create.result()

    set_progress(
        'Creating database "%s" on server "%s"...' % (database_name, server_name)
    )

    async_db_create = client.databases.begin_create_or_update(
    ...
    resource.name = "Azure Postgres - " + database_name
    resource.azure_server_name = server_name
    resource.azure_database_name = database_name
    resource.azure_database_user = server_created.administrator_login
    resource.db_address = server_created.fully_qualified_domain_name
    resource.db_port = "5432"
