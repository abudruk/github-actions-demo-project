...
def generate_options_for_configure_repository_type(field, **kwargs):
    ...
    # return [('FactoryVSTSConfiguration', 'Azure DevOps'), ('FactoryGitHubConfiguration', 'GitHub')]
...
def factory_repo_configuration(adf_client, dataFactoryObj, repo_configuration_kwargs):
    ...
    try:
        ...
        dataFactoryObj = adf_client.factories.configure_factory_repo(dataFactoryObj.location, {'factory_resource_id': dataFactoryObj.id, 'repo_configuration': repo_configuration_kwargs})
    except Exception as err:
        try:
            # delete data factory
            response = adf_client.resources.delete_by_id(dataFactoryObj.id, '2018-06-01')
