...
def verify_resource(resource_client, resource_id, api_version, not_verified=0):
    ...
    try:
        ...
        if reason in ['ResourceNotFound', 'DeploymentNotFound', 'Not Found']:
...
def run(job, *args, **kwargs):
    ...
    set_progress(f"Data Factory Delete plugin running for resource: {resource}")
    logger.info(f"Data Factory Delete plugin running for resource: {resource.name}")
