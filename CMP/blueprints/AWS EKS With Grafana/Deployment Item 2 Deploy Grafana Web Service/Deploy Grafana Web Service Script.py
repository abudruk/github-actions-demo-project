...
def _get_boto_resource_client(env_obj, service_name="ec2"):
    ...
    # get aws client object
    client = rh.get_boto3_resource(env_obj.aws_region, service_name)
...
def _create_port_forward_role(kub_obj, base_url, role_name="kube-saas:list-and-logs", namespace="grafana-namespace"):
    """
...
def _create_port_forward_role_binding(kub_obj, base_url, user_name, namespace="grafana-namespace", role_name="kube-saas:list-and-logs", role_binding_name="allow-port-forward"):
    """
...
def _create_datasources_config_map(kub_obj, base_url, namespace="grafana-namespace"):
    """
    Create a Kubernetes cluster datasources config 
    """
    # get or create namespace for aws auth config
    _get_or_create_namespaces(kub_obj, base_url, namespace)
    ...
    logger.info(f"Grafana data source config response: {rsp.json()}")
    
def _create_grafana_service(kub_obj, base_url, ec2_instance_public_ips, node_port=32000, namespace="grafana-namespace"):
    """
    Create a grafana service
    """
    # get or create namespace for grafana service
    _get_or_create_namespaces(kub_obj, base_url, namespace)
    ...
    logger.info(f"Grafana service response: {rsp.json()}")

def _create_grafana_deployment(kub_obj, base_url, namespace="grafana-namespace"):
    """
    Create a grafana deployment and pods
    """
    # get or create namespace for grafana deployments
    _get_or_create_namespaces(kub_obj, base_url, namespace)
    ...
    url = f"{base_url}/apis/apps/v1/namespaces/{namespace}/deployments"

    rsp = kub_obj.requests_post(url, json=data)

    logger.info(f"Grafana deployments response: {rsp.json()}")

def get_token(cluster_name, env_obj) -> dict:
    """
    Get eks cluster access token
    """
...
def _get_cluster_node_public_ip(kub_obj, env_obj, base_url):
    ...
    try:
        ...
        return public_ips, sg_group_ids
         ...
         logger.error(f"Grafana node list response: {rsp.json()}")
...
def create_cf_as_per_nips(index_no):
    ...
    CustomField.objects.get_or_create(
        name=f"grafana_server_endpoint_{index_no}",
        defaults={'label': f"Grafana Endpoint-{index_no}", 'type': 'URL',
...
def run(job, *args, **kwargs):
    set_progress("Starting Provision of Grafana deployments...")
    ...
    try:
        ...
        # create eks config map 
        _create_config_map(kub_obj, env_obj, base_url)

        # create grafana data sources
        _create_datasources_config_map(kub_obj, base_url)

        # create grafana service
        _create_grafana_service(kub_obj, base_url, ec2_instance_public_ips, node_port)

        # deploye grafana server
        _create_grafana_deployment(kub_obj, base_url)
    ...
    if len(ec2_instance_public_ips) == 1:
        ...
        resource.grafana_server_endpoint = "http://{0}:{1}".format(ec2_instance_public_ips[0], node_port)
        ...
        for idx, value in enumerate(ec2_instance_public_ips, start=1):
            ...
            # set endpoint value
            setattr(resource,  f"grafana_server_endpoint_{idx}", "http://{0}:{1}".format(value, node_port))

    resource.save()
    
    return "SUCCESS", "Grafana server deployed successfully.", ""